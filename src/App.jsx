...
function generatePDF(order){
  ...
  const doc = new jsPDF()
  doc.setFontSize(16)
  doc.text(`Order Receipt - ${order.customer.name}`, 10, 20)
  doc.text(`Phone: ${order.customer.phone}`, 10, 30)

  const rows = Object.entries(order.cart).map(([id,q])=>{
    const item = ITEMS.find(i=>i.id===id)
    return [item?.name || id, q, item?.price || 0, q*(item?.price||0)]
  })
  ...
  doc.text(`Subtotal: ${order.subtotal} ₹`, 10, doc.lastAutoTable.finalY+20)
  doc.save(`order_${order.timestamp}.pdf`)
}
...
function App(){
  ...
  const handleSubmit=()=>{
    ...
    const order={customer,cart,subtotal,timestamp:Date.now()}
    localStorage.setItem('lastOrder',JSON.stringify(order))
    generatePDF(order)
    alert('Order saved locally & PDF generated!')
    setCart({})
  }
      ...
      <main className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <aside className="bg-white p-4 rounded shadow">
          ...
          <div className="mt-4 text-sm text-gray-600">
            Tip: Click Place Order to save locally and download a printable PDF receipt.
          </div>
        </aside>
      </main>

      <footer className="mt-6 text-center text-sm text-gray-500">
        Designed for mobile & desktop. Works fully offline.
      </footer>
    </div>
  )
}
